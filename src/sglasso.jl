...
Compute sparse-group LASSO, group LASSO, and LASSO via proximal gradient descent.
...
function glasso(X::RealMatrix, y::RealVector, lambda::RealVector, G0::RealVector, G::RealVector; t = 0.1, K = 100) 
    T, p = size(X)
    bhat = zeros(p, 1)
    fhat = zeros(K-1, 1)
...
function lasso(X::RealMatrix, y::RealVector, lambda::RealVector; t = 0.1, K = 100) 
    T, p = size(X)
    bhat = zeros(p, 1)
    fhat = zeros(K-1, 1)
